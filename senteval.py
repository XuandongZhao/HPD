"""
Modified from
https://github.com/princeton-nlp/SimCSE/blob/main/evaluation.py
"""
import sys
import logging
import argparse
...
def print_table(task_names, scores, save_name=None):
    tb = PrettyTable()
...
def main():
    ...
    parser.add_argument("--model_type", type=str,
                        choices=['st', 'simcse'],
                        default='st',
                        help="Use sentence transformer or simcse")
    ...
    if args.mode == 'dev':
        ...
        for task in ['STSBenchmark', 'SICKRelatedness']:
            ...
            if task in results:
                ...
                scores.append("0.00")
        print_table(task_names, scores, model_name)
        ...
        task_names.append("Avg.")
        scores.append("%.2f" % (sum([float(score) for score in scores]) / len(scores)))
        print_table(task_names, scores, model_name + '-STS')
        ...
        task_names.append("Avg.")
        scores.append("%.2f" % (sum([float(score) for score in scores]) / len(scores)))
        print_table(task_names, scores, model_name + '-Transfer')
