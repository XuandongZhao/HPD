...
class SentencesDataset(Dataset):
    ...
    def __init__(self, student_model: SentenceTransformer, teacher_model: SentenceTransformer = None,
                 batch_size: int = 8, use_embedding_cache: bool = True):
        """
    ...
    def add_emb_cache(self, sentences, emb_tensor):
        assert len(sentences) == emb_tensor.shape[0]
        for i in range(len(sentences)):
            self.embedding_cache[sentences[i]] = emb_tensor[i].float().detach().numpy()
